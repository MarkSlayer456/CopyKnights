...
void calculate_main_path(unsigned int *seed, world_t *world) {
	int main_x = 0, main_y = 0; // current main path x and y cords
...
void calculate_door_masks(world_t *world) {
	...
	for(int i = 1; i <= TOTAL_ROOM_COUNT; i++) {
		...
		if(i <= CAVE_ROOM_COUNT) {
			...
			room_num -= CAVE_ROOM_COUNT;
			strcat(folder, BOG_NAME);
		} else if(i <= CATACOMBS_ROOM_COUNT) {
			room_num -= CAVE_ROOM_COUNT - BOG_ROOM_COUNT;
			strcat(folder, CATACOMBS_NAME);
		} else if(i <= ANCIENT_CITY_ROOM_COUNT) {
			room_num -= CAVE_ROOM_COUNT - BOG_ROOM_COUNT - CATACOMBS_ROOM_COUNT;
		...
		if(door_found) {
			...
			DEBUG_LOG("door found new count = %d", world->room_template_count);
		}
		close(fd);
	}
}
...
void sync_doors(world_t *world, int x, int y) {
	...
	room_t *south = y < WORLD_HEIGHT ? world->room[x][y+1] : NULL;
	room_t *east = x < WORLD_WIDTH ? world->room[x+1][y] : NULL;
...
room_t *load_room(unsigned int *seed, int x, int y, enemy_data_t *enemy_data, item_data_t *item_data, world_t *world)
{
	...
	if(room->is_main_path) {
		...
		for(int i = 0; i < world->room_template_count; i++) {
			if((room->door_mask & world->room_templates[i].mask) == room->door_mask)  {
				...
				template_count++;
				DEBUG_LOG("template count: %d", template_count);
				DEBUG_LOG("template mask: %d", templates[template_count].mask);
			}
		}
		DEBUG_LOG("%s", "main path detected!");
